...
public class IndexPatternTest {
	...
	public final void testSearchSingleTerm() throws Exception {
		...
		PatternSearch patternSearch = (IndexedPatternSearch)SearchFactory.get(PatternSearch.class, DocUtils.IDX_SEARCH);
	...
	@Test
	public final void testSearchTwoTerms() throws Exception {
		System.out.println(System.getProperty("user.dir"));
		PatternSearch patternSearch = (IndexedPatternSearch)SearchFactory.get(PatternSearch.class, DocUtils.IDX_SEARCH);
		Map<String, Long> result = patternSearch.search("Niall Ferguson".toLowerCase(), CURRENT_DIRECTORY);
		assertEquals(3, result.size());
	...
	public final void testSearchMultipleTerms() throws Exception {
		System.out.println(System.getProperty("user.dir"));
		PatternSearch patternSearch = (IndexedPatternSearch)SearchFactory.get(PatternSearch.class, DocUtils.IDX_SEARCH);
		Map<String, Long> result = patternSearch.search("areas including modern france", CURRENT_DIRECTORY);
		assertEquals(3, result.size());
		assertEquals(1, result.get("file3.txt").longValue());
	}
	
	@Test
	public final void testSearchMultipleTermsIncludingStopKeywords() throws Exception {
		System.out.println(System.getProperty("user.dir"));
		PatternSearch patternSearch = (IndexedPatternSearch)SearchFactory.get(PatternSearch.class, DocUtils.IDX_SEARCH);
		Map<String, Long> result = patternSearch.search("Guide to the Galaxy".toLowerCase(), CURRENT_DIRECTORY);
		assertEquals(3, result.size());
