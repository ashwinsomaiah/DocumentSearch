...
public class RegularExpressionPatternTest {
	...
	public final void testSearchSingleTerm() throws Exception {
		...
		PatternSearch patternSearch = (RegularExpressionPatternSearch)SearchFactory.get(PatternSearch.class, DocUtils.REGEXP_SEARCH);
		Map<String, Long> result = patternSearch.search("^The", CURRENT_DIRECTORY);
	...
	@Test
	public final void testSearchMultipleTermsWithWords() throws Exception {
		System.out.println(System.getProperty("user.dir"));
		PatternSearch patternSearch = (RegularExpressionPatternSearch)SearchFactory.get(PatternSearch.class, DocUtils.REGEXP_SEARCH);
		Map<String, Long> result = patternSearch.search("of the \\w+", CURRENT_DIRECTORY);
		assertEquals(3, result.size());
		assertEquals(2, result.get("file2.txt").longValue());
	}
	
	@Test
	public final void testSearchMultipleTermsWithNumbers() throws Exception {
		System.out.println(System.getProperty("user.dir"));
		PatternSearch patternSearch = (RegularExpressionPatternSearch)SearchFactory.get(PatternSearch.class, DocUtils.REGEXP_SEARCH);
		Map<String, Long> result = patternSearch.search("than \\d+", CURRENT_DIRECTORY);
		assertEquals(3, result.size());
		assertEquals(1, result.get("file2.txt").longValue());
		assertEquals(1, result.get("file3.txt").longValue());
	}	
